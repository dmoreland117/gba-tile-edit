...
func get_export_params(type:String) -> Dictionary:
	...
	if type == 'tilesets':
		ret = {
			...
			'array_name': {
				'type': TYPE_STRING,
				'default': Project.get_selected_tileset().tileset_name
			},
			'array_bpp': {
				'type': ExportPropContainer.TYPE_ENUM,
				'options': ARRAY_TYPES,
...
func get_preview(type:String, params) -> Control:
	...
	if type == 'palettes':
		var colors = Project.palettes.get_palette(params.get('palette', 0)).get_colors()
		var data:Array[int] = []
	...
	if type == 'tilesets':
		...
		var tile_size_prefixes = [
			'#ifndef %s_H' % tileset.tileset_name.to_upper(),
			'#define %s_H' % tileset.tileset_name.to_upper(),
